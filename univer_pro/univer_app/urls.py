...
from .views import (
    AboutUniversityViewSet, AboutCollegeViewSet, LyceumViewSet,
...
urlpatterns = [
    ...
    path('event/<int:pk>/',
         ...
         name='event detail'),
    # библиотека
    path('verify_number/', VerificationViewSet.as_view({'post': 'verify_number'}), name='verify_number'),
    path('verify_code/', VerificationViewSet.as_view({'post': 'verify_code'}), name='verify_code'),
    path('resend_code/', VerificationViewSet.as_view({'post': 'resend_code'}), name='resend_code'),
    # Руководство
